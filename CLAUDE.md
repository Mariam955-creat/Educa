...
- **Backend** : modules `user`, `course`, `enrollment`, `quiz`, `certificate`, `storage`, `language`, `ai` + `common` (erreurs, web, util) et `security` (transverses). DTO en entrée/sortie de `controller`, entités JPA jamais exposées. Un module dépend du *service* d'un autre, jamais de son repository/ses entités ; pas de cycle. Gestion d'erreurs centralisée via `@RestControllerAdvice`. Validation par `jakarta.validation`.
...
**Module admin (Should have) développé le 2026-09-15** : `AdminUserController` (`com.educa.backend.user`) — `GET /admin/users?q=` (recherche paginée nom/email), `PATCH /admin/users/{id}/roles` (`400` si rôle inconnu, `409` si un ADMIN retire son propre rôle), `PATCH /admin/users/{id}/status` (`409` si un ADMIN se désactive lui-même) ; `AuthService.refresh()` bloque désormais un compte désactivé (`401`), pas seulement le login. `AdminCertificateController` (`com.educa.backend.certificate`) — `GET /admin/certificates` (registre paginé). **Nouveau module `com.educa.backend.language`** — `Language` (table `languages`, migration `V3__languages.sql`, seed fr/en/ar actives) + `LanguageController` (`GET /languages`, public, langues actives) + `AdminLanguageController` (`GET/PATCH /admin/languages/{code}`, `404` code inconnu, `409` si dernière langue active) ; `CourseService` valide désormais la langue à la création/au **changement** de langue d'un cours (`400` si inactive) sans bloquer l'édition d'un cours existant qui garde une langue depuis désactivée. Cette table régit uniquement les **langues de contenu des cours**, pas la langue d'interface (bundles `i18n/*.json` statiques, inchangés). Frontend : `/admin` a trois onglets — Utilisateurs, Certificats, **Langues** (activer/désactiver, erreurs serveur affichées) ; `core/language/LanguageApiService` alimente dynamiquement le sélecteur de langue de `course-editor` (au lieu d'options `fr/en/ar` en dur) ; `course-editor` gère aussi désormais les erreurs serveur au save (`saveError`, absent avant — bug latent révélé par cette validation). Vérifié : `./mvnw test` → **46/46 verts** (7 nouveaux tests `AdminLanguageControllerTest`), `npm run build` + `test:ci` (13/13) OK, comportement revérifié via l'API réelle (désactiver `ar` le retire de `GET /languages` et fait échouer `POST /courses {language:"ar"}` en `400`, réactivation OK, RBAC `403`/`200` corrects).
...
**Traductions de contenu des cours (Should have, 4.9) faites le 2026-09-15** : `course_translations`/`chapter_translations` migrées (`V5__content_translations.sql`, FK vers `languages(code)`, contrairement à `courses.language` qui reste un simple `VARCHAR(2)`). Backend, module `course` (pas de module transverse) : `CourseTranslation`/`ChapterTranslation` (entités découplées, `Long courseId`/`chapterId` — même patron que `instructorId` sur `Course`, pas de relation JPA) ; `CourseTranslationService`/`CourseTranslationController` (`GET /courses/{id}/translations`, `GET/PUT/DELETE .../translations/{lang}`, propriétaire/ADMIN uniquement, `400` si langue inactive, `400` si un chapitre n'appartient pas au cours) — édition « une langue à la fois » (titre + description du cours + titre de chaque chapitre en un seul écran/un seul `PUT`, titre de chapitre vide = retire sa traduction). Lecture : `CourseService.catalog`/`getDetailBySlug` acceptent un `displayLanguage` optionnel et substituent titre/description/titres de chapitres par leur traduction si elle existe, sinon repli silencieux sur l'original (même forme de DTO, aucun champ `translated` ajouté). Frontend : `CourseApiService.detail(slug, translated=false)` — **défaut `false`, opt-in explicite** (piège évité en cours de route : le premier jet appliquait `displayLanguage` automatiquement partout, ce qui aurait fait écraser le titre original d'un cours par sa traduction affichée dès qu'un formateur ouvrait `course-editor` avec l'interface dans une autre langue — corrigé avant tout commit ; seul `course-detail` — page de consultation publique — passe `translated=true`) ; nouvel onglet « Traductions » dans `course-editor` (sélecteur de langue, champs titre/description + un champ par chapitre, enregistrer/supprimer cette langue). Vérifié : `./mvnw test` → **56/56 verts** (8 nouveaux `CourseTranslationControllerTest`), `npm run build` + `test:ci` (13/13) OK, vérifié bout-en-bout via l'API réelle et à travers le proxy Angular (traduction EN d'un cours FR : catalogue et détail renvoient bien le texte traduit avec `?displayLanguage=en`, l'original sans).

**Sniffing réel des uploads (résiduel Phase 5) fait le 2026-09-15** : la liste blanche MIME ci-dessus validait jusqu'ici le `Content-Type` **déclaré par le client** (en-tête multipart), trivialement falsifiable. `com.educa.backend.storage.FileTypeDetector` (Apache Tika `tika-core` 3.3.0, magic bytes + nom de fichier en indice) détecte désormais le type réel du contenu ; `ContentService.validateUpload` valide et stocke ce type détecté (`Content.mimeType`), jamais le `Content-Type` du client. `tika-core` seul (sans `tika-parsers`, plus lourd) suffisait — vérifié empiriquement : détection correcte de PDF/PNG/JPEG/GIF/WEBP/MP4/WEBM/ZIP par magie seule, et de `docx`/`pptx`/`xlsx`/`doc`/`xls`/`ppt` avec le nom de fichier en indice (ambigu sinon : un ZIP quelconque renommé `.docx` est accepté comme faux-positif docx — acceptable, l'essentiel est qu'un contenu dangereux, lui, n'est **jamais** mal classé même avec une extension/un `Content-Type` trompeurs, magic bytes prioritaires). Vérifié par test : `UploadSecurityTest` (2 nouveaux tests : upload PNG faussement déclaré `application/pdf` → stocké/servi comme `image/png`, pas `application/pdf` ; upload « exécutable » factice `MZ...` faussement déclaré `.pdf` → `415` révélant `application/x-msdownload`) + vérif API réelle (`curl` avec vrais fichiers). `./mvnw test` → 48/48 verts.
Backend **opérationnel et testé** (`./mvnw test` → **56 tests verts** : auth 8, course 5, course-translation 8, quiz 4, ai 9, upload-security 5, contexte 1, admin-user 7, admin-certificate 2, admin-language 7), API sur **:8081** :
- modules `user` (auth JWT + admin utilisateurs), `course` (CRUD + catalogue + publication + traductions de contenu), `enrollment` (inscription + progression %), `storage` (FS local + upload/download multipart), `quiz` (contrôle + examen final, correction auto, note pondérée 40/60, déverrouillage à 100 %), `certificate` (PDF via openhtmltopdf, n° de série, vérif publique par code, registre admin), `language` (langues de contenu des cours, actives/inactives, gestion admin), `ai` (chatbot via SDK Anthropic `anthropic-java`, repli `degraded` si clé absente/erreur) ;
...
- migrations Flyway `V1`→`V5` (`V3__languages.sql` table `languages` ; `V4__catalog_search_trgm.sql` extension `pg_trgm` + index GIN `idx_courses_title_trgm` ; `V5__content_translations.sql` tables `course_translations`/`chapter_translations`, 2026-09-15) ; `DevDataInitializer` : 4 comptes (admin, formateur, apprenant, `diplome@educa.dev`) + 2 cours de démo publiés (contrôle + examen final chacun) + parcours du compte diplômé rejoué → certificat pré-émis.
Frontend **Angular 19.2** (`npm run build` OK) : `core/` auth + courses + enrollments + quiz + certificates + ai + admin + language + i18n ; `feature/` catalog, course (page cours : contenus + progression + quiz + évaluation + certificat + widget chatbot), quiz (`quiz-take`), instructor (`course-editor` + `quiz-editor` + `course-results`), certificate (`my-certificates` + `verify/:code` public), admin (`admin-dashboard` : onglets utilisateurs/certificats/langues), dashboard.
