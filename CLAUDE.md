...
⚠️ **Régression frontend pré-existante découverte à cette occasion** (non liée au module admin, non corrigée) : `npm run test:ci` → **9/13 verts**, les 4 échecs sont dans `QuizTakeComponent` (`NullInjectorError: No provider for TranslateService`) — confirmé pré-existant sur `main` avant tout changement de cette session (probablement introduit par le retrofit i18n du 2026-09-13, `quiz-take.component.spec.ts` n'a jamais été mis à jour pour fournir `TranslateService` dans son `TestBed`). Le README/journal antérieurs à corriger (ils annoncent encore « 13 verts »).
