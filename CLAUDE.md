...
Backend **opérationnel et testé** (`./mvnw test` → **48 tests verts** : auth 8, course 5, quiz 4, ai 9, upload-security 5, contexte 1, admin-user 7, admin-certificate 2, admin-language 7), API sur **:8081** :
...
- migrations Flyway `V1`→`V4` (`V3__languages.sql` table `languages` ; `V4__catalog_search_trgm.sql` extension `pg_trgm` + index GIN `idx_courses_title_trgm`, 2026-09-15) ; `DevDataInitializer` : 4 comptes (admin, formateur, apprenant, `diplome@educa.dev`) + 2 cours de démo publiés (contrôle + examen final chacun) + parcours du compte diplômé rejoué → certificat pré-émis.
