...
**Phases 0 → 5 (MVP) terminées** (reste 1.10 Swagger ; multilingue Phase 4 validé au navigateur, chatbot IA live en attente d'une vraie clé Anthropic). **Phase 6 (rédaction finale & soutenance) en cours** : `README.md` réécrit (état réel, port 8081, comptes démo) ; `docs/05-demo-soutenance.md` créé (scénario pas à pas ~15 min). Reste : passe finale `docs/01`+`02`+`CLAUDE.md`, jeu de démo enrichi (6.2), export diagrammes Mermaid (6.4), vérif bout-en-bout (6.5), `docker-compose` de déploiement (6.6, cible à confirmer).
