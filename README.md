...
./mvnw test               # profil test → base educa_test (39 tests)
```
...
Résiduel connu (hors périmètre MVP) : clé Anthropic réelle pour le chatbot live, `pg_trgm` pour la recherche catalogue à l'échelle, sniffing de contenu des uploads (Apache Tika), gestion des langues actives côté admin (`/admin/languages`, pas de table `languages`).
