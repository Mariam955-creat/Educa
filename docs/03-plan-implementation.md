...
| # | Tâche | Fichiers / modules | Statut | MàJ |
|---|---|---|---|---|
| 5.1 | Tests d'intégration bout-en-bout des parcours critiques (auth, inscription→cours, quiz→certificat) | `backend/src/test/...` | fait (**30 tests backend** : `AuthControllerTest` 8, `CourseFlowTest` 5, `QuizFlowTest` 4, `AiChatTest` 9, `UploadSecurityTest` 3, contexte 1 ; `AiChatTest` étoffé en Phase 6 — voir journal 2026-09-10) | 2026-09-10 |
...
**Résiduel (hors MVP)** — aucun résidu de sécurité/performance restant identifié pour un déploiement public. (`server.error.include-message: always` → `never` déjà couvert par le profil `prod`, `application-prod.yml`, Phase 6.6 ; sniffing réel du contenu des uploads et `pg_trgm`/GIN pour la recherche catalogue faits le 2026-09-15, voir journal.)

---

## Phase 6 — Rédaction finale & soutenance  ·  statut : `terminée` (2026-09-14)
...
| 6.6 | Instructions de déploiement cloud (Docker) | `docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile`, `frontend/nginx.conf`, `application-prod.yml`, `.env.docker.example`, `docs/07-deploiement.md`, `README.md` | **fait et vérifié** — stack 3 conteneurs (backend JRE 25 / frontend Angular→nginx avec reverse-proxy `/api` / `postgres:18`), profil `prod` (erreurs masquées, `flyway.clean` off), frontend basculé sur API relative `/api/v1` + proxy Angular en dev (`proxy.conf.json`). `docs/07` = déploiement local + cloud (VM, base managée, PaaS, TLS). **`docker compose build` puis `up` exécutés avec succès** (2026-09-14) : le blocage précédent (« virtualisation non détectée ») était un faux négatif — Docker Desktop n'était simplement pas lancé (l'hyperviseur Windows tournait déjà, masquant les indicateurs WMI bruts). Un vrai bug trouvé et corrigé au passage : volume `db-data` à monter sur `/var/lib/postgresql` (et non `/var/lib/postgresql/data`) avec `postgres:18` (nouvelle convention `pg_ctlcluster` de l'image). Stack complète testée : `db` healthy, `backend` connecté (Flyway + Hibernate OK), `frontend` sert le SPA et le reverse-proxy `/api/v1/courses` répond `200`. Détail : journal 2026-09-14 | 2026-09-14 |
