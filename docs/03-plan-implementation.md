...
## Phase 6 — Rédaction finale & soutenance  ·  statut : `quasi terminée` (2026-09-13) — reste seulement `docker compose build`, bloqué par la virtualisation désactivée au BIOS du poste de dev
...
| 6.6 | Instructions de déploiement cloud (Docker) | `docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile`, `frontend/nginx.conf`, `application-prod.yml`, `.env.docker.example`, `docs/07-deploiement.md`, `README.md` | fait — stack 3 conteneurs (backend JRE 25 / frontend Angular→nginx avec reverse-proxy `/api` / `postgres:18`), profil `prod` (erreurs masquées, `flyway.clean` off), frontend basculé sur API relative `/api/v1` + proxy Angular en dev (`proxy.conf.json`). `docs/07` = déploiement local + cloud (VM, base managée, PaaS, TLS). ⚠️ **`docker compose build` non exécuté** : Docker Desktop est maintenant installé sur le poste, mais son daemon refuse de démarrer (« virtualisation non détectée ») — `VirtualizationFirmwareEnabled = False` confirmé en PowerShell, VT-x désactivé au BIOS/UEFI de ce laptop physique. Blocage matériel, hors de portée logicielle : nécessite que l'utilisatrice active VT-x au BIOS puis redémarre. Détail : journal 2026-09-13 | 2026-09-13 |
